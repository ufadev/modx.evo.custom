...
if (file_exists("../".MGR_DIR."/includes/config.inc.php")) {
    ...
    $avtest = @ mysql_query("SELECT alias_visible FROM {$dbase}.`{$table_prefix}site_content`");
    /// проверка для установки на мою сборку автоматическое выполнение запроса
    if ($avtest)  { 
       $output .= ' ';
    }else {
        $avadd = @ mysql_query("ALTER TABLE  {$dbase}.`{$table_prefix}site_content` ADD `alias_visible` INT( 2 ) NOT NULL DEFAULT  '1'");
        $avtest2 = @ mysql_query("SELECT alias_visible FROM {$dbase}.`{$table_prefix}site_content`");
        if ($avtest2) {
            $output .= ' ';
        }else {
